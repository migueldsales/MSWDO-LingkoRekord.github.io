...
<body>
    ...
    <!-- Add MOVE Modal -->
<div class="modal fade" id="moveAddModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Add MOVE Member</h5>
...
<!-- Edit MOVE Modal -->
<div class="modal fade" id="moveEditModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">Edit MOVE Member</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <form id="updateMOVE">
            <div class="modal-body">
                ...
                <input type="hidden" name="move_idNum" id="move_idNum" >
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                <button type="submit" class="btn btn-primary">Update MOVE Member</button>
            </div>
        </form>
        </div>
    </div>
...
<!-- View MOVE Modal -->
<div class="modal fade" id="moveViewModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">View MOVE Member</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
...
<div class="container-fluid g-0">
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header" id="move-header">
                </div>
                ...
                <div class="input-group rounded mb-3">
                    ...
                    <h4>
                        <span class="fs-2 fw-normal text-uppercase">List of MOVE (Men Opposed to Violence Against Women Everywhere) Members</span>
                        <button type="button" class="btn btn-primary float-end" data-bs-toggle="modal" data-bs-target="#moveAddModal">
                            Add MOVE Member
                        </button>
                    </h4>
                </div>
                <div class="card-body" id="main-table">
                    ...
                    <table id="moveTable" class="table table-bordered table-striped">
                        ...
                        <tbody>
                            ...
                            $query = "SELECT * FROM move_form";
                            $query_run = mysqli_query($connect, $query);

                            if(mysqli_num_rows($query_run) > 0)
                            {
                                foreach($query_run as $move)
                                {
                                    ?>
                                    <tr>
                                        <td><?= $move['ID'] ?></td>
                                        ...
                                        <td><?= $move['MiddleName'] ?></td>
                                        <td><?= $move['DateOfBirth'] ?></td>
                                        <td><?= $move['Age'] ?></td>
                                        <td><?= $move['Sex'] ?></td>
                                        <td><?= $move['CivilStatus'] ?></td>
                                        <td><?= $move['EducationalAttainment'] ?></td>
                                        <td><?= $move['Occupation'] ?></td>
                                        <td><?= $move['SkillInterest'] ?></td>
                                        <td>
                                            <button type="button" value="<?=$move['ID'];?>" class="viewMOVEBtn btn btn-info btn-sm">View</button>
                                            <button type="button" value="<?=$move['ID'];?>" class="editMOVEBtn btn btn-success btn-sm">Edit</button>
                                            <button type="button" value="<?=$move['ID'];?>" class="deleteMOVEBtn btn btn-danger btn-sm">Delete</button>
                                        </td>
                                    </tr>
                                    <?php
                                }
    ...
    <script>
        $(document).on('submit', '#saveMOVE', function (e) {
            e.preventDefault();

            var formData = new FormData(this);
            formData.append("save_MOVE", true);
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if(res.status == 422) {
                        ...
                        $('#moveAddModal').modal('hide');
                        $('#saveMOVE')[0].reset();

                        alertify.set('notifier','position', 'top-right');
                        alertify.success(res.message);

                        $('#moveTable').load(location.href + " #moveTable");
        ...
        $(document).on('click', '.editMOVEBtn', function () {

            var move_id = $(this).val();
            
            $.ajax({
                type: "GET",
                url: "move-code.php?move_id=" + move_id,
                success: function (response) {
                    ...
                    if(res.status == 404) {
                        ...
                        $('#moveEditModal').modal('show');
                    }
                }
            });
        });

        $(document).on('submit', '#updateMOVE', function (e) {
            e.preventDefault();

            var formData = new FormData(this);
            formData.append("update_MOVE", true);
        ...
        $(document).on('click', '.viewMOVEBtn', function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if(res.status == 404) {
                        ...
                        $('#moveViewModal').modal('show');
                    }
                }
            });
        });

        $(document).on('click', '.deleteMOVEBtn', function (e) {
            e.preventDefault();
